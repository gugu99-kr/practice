...
if uploaded_csv is None or uploaded_excel is None:
    st.warning("👉 CSV + 엑셀 둘 다 업로드하세요")
    st.stop()

df = pd.read_csv(uploaded_csv, encoding="cp949")
df_stat = pd.read_excel(uploaded_excel)
...
if csv_gu_col is None or excel_gu_col is None:
    st.error("❌ 구(시군구) 컬럼 찾기 실패")
    st.write("CSV 컬럼:", df.columns)
    st.write("엑셀 컬럼:", df_stat.columns)
    st.stop()
...
selected_gu = st.sidebar.selectbox("자치구", ["전체"] + list(gu_list))
